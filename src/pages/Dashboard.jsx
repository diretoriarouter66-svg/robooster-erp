...
import { Package, Users, Ship, ShoppingCart, DollarSign, AlertTriangle, TrendingUp, Truck } from "lucide-react";
...
export default function Dashboard() {
  ...
  const loadDashboard = async () => {
    try {
      const [products, customers, suppliers, imports, orders, financial] = await Promise.all([
        base44.entities.Product.list("-created_date", 200),
        base44.entities.Customer.list("-created_date", 200),
        base44.entities.Supplier.list("-created_date", 200),
        base44.entities.ImportProcess.list("-created_date", 50),
      ...
      const activeImportsList = imports.filter(i => !["completed", "cancelled"].includes(i.status));
      ...
      setStats({
        ...
        activeImports: activeImportsList.length,
        pendingReceivables,
        pendingPayables,
      ...
      setActiveImports(activeImportsList.slice(0, 5));
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        ...
        <StatCard icon={Ship} label="Importações Ativas" value={stats?.activeImports || 0} color="warning" />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        ...
        <StatCard icon={AlertTriangle} label="Estoque Baixo" value={stats?.lowStockCount || 0} color={stats?.lowStockCount > 0 ? "destructive" : "success"} />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-card rounded-xl border border-border p-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-heading font-semibold text-sm">Importações Ativas</h3>
            <Link to="/imports" className="text-xs text-primary hover:underline">Ver todas</Link>
          </div>
          {activeImports.length === 0 ? (
            <p className="text-sm text-muted-foreground py-4 text-center">Nenhuma importação ativa</p>
          ) : (
            <div className="space-y-2">
              {activeImports.map((imp) => (
                <div key={imp.id} className="flex items-center justify-between py-2 px-2 rounded-lg hover:bg-muted/50 transition-colors">
                  <div>
                    <p className="text-sm font-medium">{imp.reference}</p>
                    <p className="text-xs text-muted-foreground">{imp.supplier_name || "—"}</p>
                  </div>
        ...
        {lowStock.length > 0 && (
          <div className="bg-card rounded-xl border border-destructive/20 p-4 lg:col-span-2">
